...
| the special characters like ｮｷ are represented in unicode by char type 2 (32 bits) which is referred to as surrogate pair.
...
.. code-block:: java
   ...
   System.out.println(subStr); // => "ｮｷ"

| In the example above, when you try to fetch "ｮｷ田" by taking out 2 characters from 0th character (beginning), only "ｮｷ" could be fetched since the surrogate pair is represented by 32 bits (char type 2).
...
.. code-block:: java
   ...
   String subStrSurrogate = str.substring(startIndexSurrogate, endIndexSurrogate); // => "ｮｷ田"
