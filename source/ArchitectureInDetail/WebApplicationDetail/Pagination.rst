...
| 処理のフローは、以下の通りである。
 ...
 .. list-table::
    ...
    * - | (2)
      - | 検索条件とデフォルトのページ検索用のリクエストパラメータをフォームセッションオブジェクトに格納及び検索処理を行う。
    * - | (3)
      - | フォームセッションオブジェクトにページ検索用のリクエストパラメータを格納及びページ検索処理を行う。詳細から遷移した場合の検索処理を行う。
    * - | (4)
      - | Article詳細の検索処理を行う。
...
- Controller
 ...
 .. list-table::
    ...
    * - | (7)
      - | ページリンクを押下した際はGETで送信する。フォームセッションオブジェクトの \ ``page`` \ 、\ ``size`` \ が格納される。
    * - | (8)
 ...
 .. note:: **セッションオブジェクトの削除について**
    ...
    * 入力画面を初期表示するためのリクエスト。(任意)
    ...
    本実装例では検索一覧画面の初期表示リクエストでフォームセッションを削除しているが、アプリケーション及びシステム要件を考慮してセッションに格納したオブジェクトを削除すること。詳細については、「\ :doc:`SessionManagement`\ のセッションに格納したオブジェクトの削除」を参照されたい。
