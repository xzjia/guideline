...
- URL方式の制約
 ...
 共通ライブラリから提供している \ ``<t:pagination>`` \ を使ってクエリ文字列を生成することができる。
 ただし、URLにページネーション情報を含める方式で実現した場合の制約は、以下の通りである。

  * | 詳細画面などのページネーション以外の画面に遷移した場合、そのあと遷移元の場面へ戻ることができない。
  * | センシティブな情報が検索条件に含まれる場合、URLに表示、ログに出力、インターネットを中継する第三者に見られる可能性がある。
  * | 検索項目が非常に多い場合、URLの文字数制約でページングすることができない。


- セッション方式の制約

 ページネーション情報をセッションに格納することで、複数の画面(複数のリクエスト)をまたがって、簡単にデータを持回ることができる。
 ただし、セッションを用いた方式で実現した場合の制約は、以下の通りである。


  * | 同一処理の画面を、複数のブラウザやタブで立ち上げた場合、互いの操作がセッション上に格納しているデータに干渉しあうため、データの整合性を保つことができなくなる。
    | そして、データの整合性を保つための制御は、ユーザビリティの低下させてしまう。
...
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
| :ref:`ページネーション利用時の制約 <Constraints_at_the_time_of_pagination_use>` のURL方式の制約として示している通り、\ ``<t:pagination>``\ の \ ``criteriaQuery`` \属性 や \ ``queryTmpl`` \属性 で作成したクエリ文字をを使用してリクエストパラメータで引き継ぐ方法には課題がある。
| 課題は、以下の通り。

  * 一覧画面から派生した別画面（詳細画面など）に遷移してから戻る際に遷移前の一覧画面を表示するための検索条件、ソート条件、表示ページ数が保持できない
  * URLにリクエストパラメータがすべて表示されるため、センシティブな情報を検索条件に使用できない
  * URLに文字数制限がある場合では、検索項目が多いと検索条件を引き継ぐことができない

| そのため、検索条件、ソート条件、ページネーション情報をセッションへ格納して画面間で持ち回る方法で課題を解決することができるが、セッション方式の制約として示している課題を踏まえてどちらの方式を採用するか設計する必要がある。
| 課題は、以下の通り。

  * セッションに格納したデータの整合性をとるための対策でアプリケーションのユーザビリティが低下
  * セッションに格納するデータの量に比例して、APサーバのメモリ使用量が増大
  * データをセッションに格納するため、APサーバのスケーラビリティを低下
